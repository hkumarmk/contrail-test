...
Build Contrail-test ci container
  ...
  -r|--repo         Contrail-test git repo, Default: github.com/juniper/contrail-test-ci.git
  -b|--branch       Contrail-test git branch, Default: master
  --fab-repo        Contrail-fabric-utils git repo
  --fab-branch      Contrail-fabric-utils git branch, Default: master
  --ci-repo	        Contrail-test-ci git repo, Default: github.com/juniper/contrail-test.git
  --ci-branch       Contrail-test-ci branch, Default: master
 ...
 Example:
  ...
  $0 -r https://github.com/hkumarmk/contrail-test -b working --ci-repo https://\$GITUSER:\$GITPASS@github.com/juniper/contrail-test-ci -e /tmp/export2 http://nodei16/contrail-install-packages_2.21-105~juno_all.deb

EOF
}

if ! options=$(getopt -o hnr:b:u:t:d:e: -l help,no-cache,ci-repo:,ci-branch:,export:,dockerfile:,repo:,branch:,contrail-install-package-url:,fab-repo:,fab-branch:,container-tag: -- "$@"); then
# parse error
  usage
  exit 1
fi
...
while [ $# -gt 0 ]; do
  case "$1" in
		-h|--help) usage; exit;;
		-r|--repo) CONTRAIL_TEST_REPO=$2; shift;;
		-b|--branch) CONTRAIL_TEST_BRANCH=$2; shift;;
		...
		--ci-branch) CONTRAIL_TEST_CI_BRANCH=$2; shift;;
...
if [[ -z $CONTAINER_TAG ]]; then
    if [[ ! $contrail_install_package_url =~ http[s]*://.*/contrail-install-packages_[0-9\.\-]+~[a-zA-Z]+_all.deb ]]; then
        ...
        CONTAINER_TAG=contrail-test-${openstack_release}:${contrail_version}
    fi
fi
...
docker build ${cache_opt} -t ${CONTAINER_TAG} -f $DOCKERFILE \
    ...
    --build-arg CONTRAIL_TEST_REPO=$CONTRAIL_TEST_REPO  \
    --build-arg CONTRAIL_TEST_BRANCH=$CONTRAIL_TEST_BRANCH \
